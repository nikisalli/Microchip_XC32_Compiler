...
/* MERGE-TODO: these are hopefully completely redundant. */
#ifndef PIC32
#define PIC32
#endif
...
static void pragma_vector2func_decl(const struct vector_dispatch_spec*);
...
#define VECTOR_DISPATCH_FNAME     "__vector_dispatch_"
...
} valid_section_flags[] =
{
  ...
  {
    SECTION_ATTR_BSS, 'b',
    SECTION_BSS,     SECTION_CODE | SECTION_PERSIST |
    SECTION_READ_ONLY
  },
  {
    SECTION_ATTR_CODE, 'x',
  ...
  {
    SECTION_ATTR_DATA, 'd',
    SECTION_WRITE, SECTION_READ_ONLY
  },
  ...
  },
  {
    ...
    SECTION_PERSIST,  SECTION_CODE | SECTION_BSS |
    SECTION_MERGE |
...
{
  /* the reserved section names are matched in the order of appearance below;
   * the ones ending in '.' will match any name with that prefix while
   * the others only the entire name */
  { ".bss",    SECTION_BSS },
...
#if 1
static unsigned int mchp_load_resource_info(const char *id, char **matched_id, const char* mchp_resource_file_in);
static unsigned int mchp_load_resource_info(const char *id, char **matched_id, const char* mchp_resource_file_in) {
  struct resource_introduction_block *rib;
  struct resource_data d;
  const char* mchp_resource_file_in_di;
  ...
  mchp_resource_file_in_di = mchp_resource_file_in+strlen(mchp_resource_file_in)-strlen("xc32_device.info");

  /* read_device_rib assumes that the resource file name is xc32_device.info. Ensure that it is. */
  if (strcmp (mchp_resource_file_in_di, "xc32_device.info") != 0)
  {
...
unsigned int validate_device_mask (const char *id, char **matched_id, const char* mchp_resource_file_in);
unsigned int validate_device_mask (const char *id, char **matched_id, const char* mchp_resource_file_in)
{
...
mchp_subtarget_override_options(void)
{
  ...
  validate_device_mask (mchp_processor_string, &mchp_target_cpu_id,
                              mchp_resource_file);
#endif
}
...
mchp_output_ext_region_memory_info (void)
{
    ...
    for (m = pic32_external_memory_head; m; m = m ->next) {
      sprintf(buff, ".memory _%s, origin(0x%x), size(0x%x)", m->name, m->origin, m->size);
      symtab->finalize_toplevel_asm (build_string(strlen(buff), buff));
    }
}
...
void
pic32_emit_su_section (const char *name)
{
                       ...
                       int flags ATTRIBUTE_UNUSED, bool *no_add_attrs)
{
  ...
  if (flag_generate_lto && (flag_pic || flag_pie))
    {
                          ...
                          int flags ATTRIBUTE_UNUSED, bool *no_add_attrs)
{
  ...
  set_decl_section_name(decl, scn_name);
...
mchp_output_vector_dispatch_table (void)
{
  ...
  for (dispatch_entry = vector_dispatch_list_head ;
    ...
    {
      pragma_vector2func_decl(dispatch_entry);
    }
}
  ...
  if (TREE_CODE (decl) == FUNCTION_DECL)
    {
      ...
      bool micromips_p, vector_p, longcall_p;
  ...
  if (TREE_CODE(*decl) == FUNCTION_DECL)
    {
      ...
      if ((pic32_device_mask & HAS_L1CACHE) &&
          ((address_val < 0xC0000000ul) && (address_val > 0xA0000000ul)))
        {
          ...
          TREE_INT_CST_ELT(address,0) = address_val;
        }
     }
...
tree mchp_keep_attribute(tree *node, tree identifier ATTRIBUTE_UNUSED,
                                ATTRIBUTE_UNUSED tree args, int flags ATTRIBUTE_UNUSED,
                                ATTRIBUTE_UNUSED bool *no_add_attrs)
{
  DECL_COMMON (*node) = 0;
...
bool
mchp_function_profiling_epilogue (ATTRIBUTE_UNUSED bool sibcall_p)
{
  ...
  /* MERGE-NOTES: double-check that mchp_function_profiling_epilogue results is not used anywhere  */
  return false;
}
      ...
      if (cfun->machine->interrupt_handler_p)
        {
          ...
          if (SOFTWARE_CONTEXT_SAVE == cfun->machine->current_function_type)
            ...
            {
              gcc_assert (SHADOW_SET_YES == cfun->machine->use_shadow_register_set);
              /* Restore the registers.  */
              if (TARGET_HARD_FLOAT && cfun->machine->frame.fmask)
                {
                  /* Restore the FPU FCSR control and status register */
...
void
mchp_output_function_epilogue (FILE *file)
{
              ...
              if (cfun->machine->interrupt_priority == 7)
                {
                  ...
                  cfun->machine->use_shadow_register_set = SHADOW_SET_YES;
...
mchp_subtarget_save_reg_p (unsigned int regno)
{
  ...
  if (cfun->machine->current_function_type == SAVEALL_CONTEXT_SAVE)
    {
      gcc_assert(GP_REG_FIRST == 0 && "regno >= GP_REG_FIRST should always evaluated to true");

      if (1/*regno >= GP_REG_FIRST*/ && regno <= GP_REG_LAST)
        return true;
...
void
mchp_output_function_prologue (FILE *file)
{
  if (cfun->machine->current_function_type == SRS_CONTEXT_SAVE)
...
int mips_builtin_decl_index (int idx);
static void
pragma_vector2func_decl (const struct vector_dispatch_spec *dispatch_entry)
{

  /*i386.c/make_resolver_func was used as model for building an artificial
   * func*/
  ...
  */
  ...
  gcc_assert (dispatch_entry->vector_number < 256);

  sprintf (fname, VECTOR_DISPATCH_FNAME"%d", dispatch_entry->vector_number);

  // build a function-decl of type: void func (void)
  tree ftype
      = build_function_type_list (void_type_node, /* no args */ NULL_TREE);
  tree fndecl = build_fn_decl (fname, ftype);

  SET_DECL_ASSEMBLER_NAME (fndecl, get_identifier (fname));
  sprintf (sname, ".vector_%d", dispatch_entry->vector_number);
  set_decl_section_name (fndecl, sname);

  // no prologue & epilogue must be generated for these __vector_dispatch_
  // routines
  tree naked_attrib = build_tree_list (get_identifier ("naked"), NULL_TREE);
  // exception handlers are always MIPS32 microMIPS
  tree nomips16_attrib
      = build_tree_list (get_identifier ("nomips16"), NULL_TREE);
  ...
  // generate call expr to __builtin__j_call(dispatch_entry->target)
  push_struct_function (fndecl);
  gcc_assert (cfun && "cfun should have been set by push_struct_function");
  gcc_assert ((strcmp (function_name (cfun), fname) == 0)
              && "__vector_dispatch_ function expected");
  cfun->machine->vector_dispatch = 1;
  tree target = build_fn_decl (
      "__builtin__jump",
  ...
  DECL_SAVED_TREE (fndecl)
      = build_call_expr (target, 1,
                         build_string_literal (strlen (dispatch_entry->target),
                                               dispatch_entry->target));

  bool use_long_call_attribute = false;

  if (TARGET_LONG_CALLS)
    use_long_call_attribute = true;
  else if (dispatch_entry->longcall)
    use_long_call_attribute = true;
  else
    {
      switch (dispatch_entry->isr_isa_mode)
        {
        case pic32_isa_unknown:
        case pic32_isa_mips16e:
          use_long_call_attribute = true;
          break;

        case pic32_isa_micromips:
          use_long_call_attribute
              = !(mips_base_compression_flags & MASK_MICROMIPS);
          break;

        case pic32_isa_mips32r2:
          use_long_call_attribute
              = (mips_base_compression_flags & (MASK_MIPS16 | MASK_MICROMIPS));
          break;

        default:
          gcc_assert (0 && "Invalid ISA MODE");
          use_long_call_attribute = true;
          break;
        }
    }

  if (use_long_call_attribute)
    DECL_FUNCTION_CODE (target)
        = (built_in_function)mips_builtin_decl_index (CODE_FOR_pic32_jump_to_addr_in_reg);
  else
    DECL_FUNCTION_CODE (target)
        = (built_in_function)mips_builtin_decl_index (CODE_FOR_pic32_jump_to_symbol);
  DECL_BUILT_IN_CLASS (target) = BUILT_IN_MD;

  // gimplfy newly created function
  gimplify_function_tree (fndecl);

  // insert newly created function in call graph
  cgraph_node::add_new_function (fndecl, true);
  symtab->call_cgraph_insertion_hooks (cgraph_node::get_create (fndecl));
...
/* XC32-1686: 
   This contains the smart-io match logic formerly tacked into mchp_strip_name_encoding. 

   Given a NOT-decorated name (no name encoding), return the matching smart-io symbol if 
   needed, or the name itself otherwise.
*/
static const char *
...
{
  return maybe_get_smartio_name (mchp_strip_name_encoding_helper (symbol_name));
}
                                   ...
                                   mchp_interesting_fn *matching_fn)
{
  ...
  /* XC32-1520: having seen a call with a format arg, we can now indicate that
...
/*
  ...
  Copy-paste this routine from XC32 gcc/gcc/rtlanal.c file
 */
static rtx
...
/*
  pic32_jump_to_symbol & pic32_jump_to_addr_in_reg patterns are ONLY meant to
  be used to generate jump instructions from __vector_dispatch_ functions
  (XC32-1618)
 */
int
mchp_is_vector_dispatch_call ()
{
  return (strncmp (current_function_name (), VECTOR_DISPATCH_FNAME,
                   strlen (VECTOR_DISPATCH_FNAME))
          == 0);
}
  ...
  if (mchp_clear_fn_list)
    {
	  ...
	  /* XC32-1520: we used to mark everything as 'convertable' by
	     default, which causes us to use the '_0'-suffix form in the cases
             where we don't see a call with a format string.
	     We now assume the worst and set this only if we see an actual call to it
             for smart-io=1. For smart-io=2, we want to use the _0-form in such cases. */
  ...
  if (decl)
    {
      ...
      if (a)
      ...
      else
        {
         ...
         {
           is_default = 0;
           f+= sprintf(f,",memory(_%s)", TREE_STRING_POINTER (memory));
#if 0
...
match_reserved_sec_name (const char *sec_name)
{
  struct reserved_section_names_ *r = reserved_section_names;

  for (; r && r->section_name; ++r)
    {
      const size_t len = strlen (r->section_name);
      if (len
          && !strncmp (sec_name, r->section_name, len)/* first 'len' chars match */
          && (!sec_name[len]                          /* then a NUL => exact match */
              || (sec_name[len-1] == '.'              /* or a prefix match + at */
                  && sec_name[len])))                 /* least one other char */
        return r;
    }
...
struct valid_section_flags_ *
match_valid_sec_flags_name (const char *flag_name)
{
  struct valid_section_flags_ *f = valid_section_flags;

  for (; f && f->flag_name; ++f)
    /* NB: matches the beginning/prefix of 'flag_name'
     * (probably b/c the input can contain other flags also) */
    if (!strncmp (flag_name, f->flag_name, strlen (f->flag_name)))
      return f;
    ...
    SECTION_FLAGS_INT attr_flags)
{
  ...
  if (name)
    {
      r_section = match_reserved_sec_name (name);
    }
  if (r_section)
    {
  ...
  if (f) do
      {
        ...
        if (*f)
          {
            /* FIXME: the valid_section_flags[] array contains two entries with the
             * same ("code") flag name (SECTION_ATTR_CODE/'x' and SECTION_ATTR_CONST/'r');
             * this will always match the first one i.e. SECTION_ATTR_CODE/'x' */
            v_flags = match_valid_sec_flags_name (f);
            if (!v_flags)
              {
                if (first_flag)
                  {
...
mchp_build_prefix (tree decl, int fnear, char *prefix, SECTION_FLAGS_INT flags)
{
  ...
  noload_attr = lookup_attribute ("noload", DECL_ATTRIBUTES (decl));
  ...
  if (DECL_SECTION_NAME (decl))
    {
      ...
      if ((flags & SECTION_WRITE)
        ...
        {
          if (TREE_CODE (decl) == VAR_DECL && mchp_persistent_p (decl))
  ...
  if (TREE_CODE (decl) == VAR_DECL)
    {
	 ...
	 XC32-1578: Also avoid altering the flags of the .gnu.linkonce sections */
	  ...
	  && (HAVE_COMDAT_GROUP || !(flags & SECTION_LINKONCE)))
	{
      ...
      if ((flags & SECTION_PERSIST)  || (mchp_persistent_p(decl)))
        {
          /* clean up BSS possibly being set because of the declaration. */
	  flags &= ~(SECTION_FLAGS_INT) SECTION_BSS;
          flags |= SECTION_PERSIST;
  ...
  if ((flags & SECTION_WRITE) || is_ramfunc
      || (space_attr
	  && (get_identifier ("data")
	      == (TREE_VALUE (TREE_VALUE (space_attr))))))
    {
  ...
  if (mchp_coherent_p (decl) || (flags & SECTION_PERSIST)
                        ...
                        int reloc ATTRIBUTE_UNUSED)
{
  ...
  if (name)
  {
    ...
    {
      flags = (flags & ~(SECTION_FLAGS_INT)SECTION_WRITE) | SECTION_CODE;
    }
  }
  ...
  /* SECTION_MERGE doesn't really map to our attributes. */
  if ((flags & SECTION_MERGE))
    flags = (flags & ~(SECTION_FLAGS_INT)SECTION_WRITE) | SECTION_READ_ONLY;
                           ...
                           int rounded)
{
  ...
  SECTION_FLAGS_INT flags = validate_identifier_flags(name, NULL_TREE, 0);
...
const char * mchp_sdata_section_asm_op(void)
{
  ...
  return "\t.section\t.sdata,data,near";
}
...
validate_identifier_flags (const char *prefix, tree decl, SECTION_FLAGS_INT flags)
{
  ...
  while (f && *f == MCHP_EXTENDED_FLAG[0])
	...
	{
	  flags |= SECTION_BSS;
          flags &= ~(SECTION_FLAGS_INT) SECTION_PERSIST;
...
void pic32_final_include_paths(struct cpp_dir* quote, ATTRIBUTE_UNUSED struct cpp_dir* bracket)
{
  struct cpp_dir *p;
...
static void mchp_update_frp_info (const char *name)
{
  ...
  {
    if (strncmp(m->name, name, strlen(name)) == 0)
...
static int
mchp_get_strip_length (const char *str)
{
  static const char extended_flag_ch = MCHP_EXTENDED_FLAG[0];

  gcc_assert(str && "Invalid function name");
  ...
  switch (*str)
    {
      ...
      case extended_flag_ch:
      {
        const char *pch = strrchr(str, extended_flag_ch);

        gcc_assert(pch && "Invalid extended arg format ?");
        gcc_assert(pch > str);
...
void
mchp_asm_output_labelref (FILE *stream, const char *name)
{
  int skip;
  int verbatim = 0;

  while ((skip = mchp_get_strip_length (name)))
    {
      verbatim |= (*name == '*');
      name += skip;
    }

  /* XC32-1686: check if we have a smartio name */
